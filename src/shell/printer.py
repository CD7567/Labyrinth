...
class Printer:
    ...
    def print_labyrinth(self) -> None:
        ...
        print(f'Start cell: {self.__labyrinth.start_tile}')
        print(f'Finish cell: {self.__labyrinth.finish_tile}')
        print(f'Algo: {self.__labyrinth.algo}', '\n')

        for j in range(self.__labyrinth.width):
            print(' ', self.__path['3'] if (j, 0) == self.__labyrinth.start_tile else ' ', sep='', end='')
        print(' ')
        ...
        for i in range(self.__labyrinth.height):
            if i == 0:
                for j in range(self.__labyrinth.width):
                    if j == 0:
                        print(self.__border['1100'
                                            + str(self.__labyrinth.board[j][i].top)
                                            + str(self.__labyrinth.board[j][i].left)],
                              self.__border['101010'] if self.__labyrinth.board[j][i].top else ' ',
                              sep='', end='')
                    else:
                        print(self.__border['10'
                                            + str(self.__labyrinth.board[j - 1][i].top)
                                            + '0' + str(self.__labyrinth.board[j][i].top)
                                            + str(self.__labyrinth.board[j][i].left)],
                              self.__border['101010'] if self.__labyrinth.board[j][i].top else ' ',
                              sep='', end='')

                print(self.__border['001001'] if self.__labyrinth.board[-1][i].top else self.__border['010101'])

            else:
                for j in range(self.__labyrinth.width):
                    if j == 0:
                        print(self.__border['010'
                                            + str(self.__labyrinth.board[j][i - 1].left)
                                            + str(self.__labyrinth.board[j][i].top)
                                            + str(self.__labyrinth.board[j][i].left)],
                              self.__border['101010'] if self.__labyrinth.board[j][i].top else ' ',
                              sep='', end='')
                ...
                print(self.__border['001101'] if self.__labyrinth.board[-1][i].top else self.__border['010101'])

            for j in range(self.__labyrinth.width):
                print(self.__border['010101'] if self.__labyrinth.board[j][i].left else ' ',
                      self.__path[self.__labyrinth.board[j][i].type],
                      sep='', end='')
            print(self.__border['010101'])

        for j in range(self.__labyrinth.width):
            print(self.__border['000110'] if j == 0 else (
                self.__border['001110'] if self.__labyrinth.board[j][-1].left else self.__border['101010']),
